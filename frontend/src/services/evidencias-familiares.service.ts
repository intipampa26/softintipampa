...
export interface EvidenciaFamiliar {
  id: number;
  productorId: number;
...
export interface UploadEvidenciaFamiliarDto {
  productorId: number;
  nombreArchivo: string;
  file: File;
}
...
function writeCache(productorId: number, data: EvidenciaFamiliar[]) {
  localStorage.setItem(cacheKey(productorId), JSON.stringify(data));
}

export const evidenciasFamiliaresService = {
  async getByProductor(productorId: number): Promise<EvidenciaFamiliar[]> {
    if (!navigator.onLine) return readCache(productorId);
    try {
      const { data } = await api.get<EvidenciaFamiliar[]>(BASE, { params: { productorId } });
  ...
  async upload(dto: UploadEvidenciaFamiliarDto): Promise<EvidenciaFamiliar | null> {
    const validationError = validateFile(dto.file);
    if (validationError) throw new Error(validationError);

    if (!navigator.onLine) {
      const fake: EvidenciaFamiliar = {
        id:               Date.now(),
    ...
    const { data } = await api.post<EvidenciaFamiliar>(`${BASE}/upload`, form, {
  ...
  async remove(id: number, productorId: number): Promise<void> {
    if (!navigator.onLine) {
      syncService.enqueue({ url: `${BASE}/${id}`, method: 'DELETE', module: 'evidencias-familiares' });
